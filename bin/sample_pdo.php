...
try {
    ...
    $query = 'INSERT INTO `easytest` SET `name` = :name';
    $lastInsertId1 = $dataProvider->insert($query, ['name' => 'Easy']);
    ...
    $affectedRows = $dataProvider->update($query, ['Easy MySQL', $lastInsertId1]);
    ...
    $query = 'SELECT * FROM `easytest` WHERE `bad-name` = 1';
    try {
    ...
    $query = 'SELECT * FROM `easytest` WHERE name LIKE :name';
    $array = $dataProvider->fetchAllAssociative($query, ['name' => '%E%']);
    print_r($array);
    echo "\n";

    echo (++$i) . ". fetchFirstColumn with binding: \n";
    $query = 'SELECT name FROM `easytest` WHERE name LIKE :name';
    $array = $dataProvider->fetchFirstColumn($query, ['name' => '%E%']);
    print_r($array);
    echo "\n";
